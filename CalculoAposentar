...
def se_mulher(idade_mulher): #funcao para calcular o tempo restante se for mulher
  mulher_idade_minina = 62
  restante = mulher_idade_minina - idade_mulher
  mostrar_tempo(restante)

def se_homem(idade_homem): #funcao para calcular o tempo restante se for homem
  homem_idade_minima = 65
  restante = homem_idade_minima - idade_homem
  mostrar_tempo(restante)

def mostrar_tempo(resta): #funcao para mostrar o tempo restante
...
idade = int(input('Sua idade:')) #Coletando idade da pessoa
sexo = str(input('Agora nos infome seu sexo, se for homem responda "H", se for mulher responda "M":')) #Coletando sexo da pessoa

if sexo == 'M': 
  se_mulher(idade) #Se for mulher chama a funcao se_mulher
else: 
  se_homem(idade) #Se for homem chama a funcao se_homem
